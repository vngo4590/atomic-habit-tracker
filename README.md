...
- New habit builder using an inline Mad-Libs implementation intention sentence with schedule presets and time-block selection. A user may keep at most **3 active habits** at a time; inducting a habit into the Hall of Fame (or archiving it) frees a slot, and a reusable `HelpTip` explains the rule at the point of creation.
...
- **Data backup & restore.** Settings → Data offers a full account export and import. Export downloads an authoritative, versioned JSON envelope built server-side from the database at `GET /api/v1/export` (`lib/backup/export.ts`). Import merges a previously exported file: it validates against `lib/contracts/backup.ts`, then upserts every record by id / natural key inside one transaction via `mergeBackup` (`lib/backup/import.ts`) — additive only, never deleting existing data, and idempotent on re-import. Pets are exported for portability but intentionally **not** imported (the ecosystem is procedural, mortal, and adoption-capped). The flow runs through `importDataAction` (`lib/actions/backup.ts`).
- **Active-habit cap.** A user may have at most **3 active habits** at once. "Active" means not archived (`archivedAt == null`) and not inducted into the Hall of Fame (no `FormationVerdict` with `decision = formed`). The rule is enforced server-side as the source of truth: `createHabit` (`lib/repositories/habits.ts`) counts active habits via `countActiveHabits` and returns a discriminated `{ ok: false, reason: "cap" }` result (mirrored by `createHabitAction` and a 409 from `POST /api/v1/habits`). The client mirrors it through the shared predicate in `lib/habit-cap.ts` (`MAX_ACTIVE_HABITS`, `activeHabitCount`, `remainingHabitSlots`): the new-habit page disables submission and explains the cap, and the optimistic store rolls back with a Toast on refusal. Inducted habits stay trackable but free a slot; existing over-cap data is grandfathered (never force-archived).
- The active OpenSpec change is `max-3-active-habits`.
