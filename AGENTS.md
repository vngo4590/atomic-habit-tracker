...
- Active-habit cap: a user may keep at most **3 active habits** (`MAX_ACTIVE_HABITS`). "Active" = not archived AND not inducted (no `FormationVerdict` with `decision = formed`). The shared predicate lives in `lib/habit-cap.ts` (`activeHabitCount`, `remainingHabitSlots`); the server enforces it authoritatively in `createHabit` (`lib/repositories/habits.ts`, via `countActiveHabits`), returning a discriminated `{ ok: true, habit } | { ok: false, reason: "cap" }` result surfaced by `createHabitAction` and a 409 from `POST /api/v1/habits`. The client mirrors it on `/habits/new` (disabled submit + cap banner + reusable `components/HelpTip.tsx`), and the optimistic `addHabit` store path rolls back with a Toast on refusal. Inducting or archiving a habit frees a slot; existing over-cap data is grandfathered (never force-archived).
