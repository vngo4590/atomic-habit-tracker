...
- Today page (`/`): shows only habits scheduled for today that are not yet done. No Morning/Afternoon/Evening grouping. Each card shows check circle, name+identity, streak, and 30-day progress. Supports habit search across all habits. Stacked habits are revealed sequentially — only the first undone habit in each chain is visible; checking it off reveals the next.
