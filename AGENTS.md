...
- Pet Ecosystem (`/pet`): a procedural, evolving, mortal Tamagotchi world. The pure engine lives under `lib/pet/` (`genome.ts` seeded PRNG + temperaments, `sprite.ts` deterministic pixel-art generator, `evolution.ts` life-stage ladder, `simulation.ts` real-time satiety/health decay with permanent death, `mood.ts` mood + idle-animation derivation, `index.ts` barrel with `buildPetView`). There are NO hardcoded pets — appearance is a pure function of `(seed, temperament, stage)`. Pets are persisted in PostgreSQL via `lib/repositories/pets.ts` (`Pet` + `PetFeedLog` models), mutated through `lib/actions/pets.ts` (`adoptPetAction`, `feedPetAction`, `buryPetAction`) with Zod contracts in `lib/contracts/pet.ts`, and loaded into the store by `getStoreSnapshot`. Food is a shared daily pool: each habit completed today earns one unit, spent across all pets (capped at 3 alive). UI lives in `app/(root)/pet/` (`page.tsx`, `PetEcosystem.tsx`, `PetCard.tsx`, `AdoptPanel.tsx`); `components/pet/MoodSprite.tsx` adds Framer Motion idle loops over `components/pet/PixelSprite.tsx`.
