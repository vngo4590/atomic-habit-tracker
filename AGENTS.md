...
- Data model: authenticated user data lives in PostgreSQL through Prisma repositories and is loaded from `app/(root)/layout.tsx` with `getStoreSnapshot(userId, todayKey())`. `components/StoreProvider.tsx` and `lib/store.ts` manage in-memory optimistic cache state around server actions; they are not a browser persistence layer. Browser `localStorage` is limited to local UI mirrors such as `atomicly:theme`, `atomicly:accent`, `atomicly:theme-variant` (the selected named theme: light/dark/glass/neon/fairy/stars), and `atomicly:pet` (the Pet Companion's chosen character + per-day feed counts).
- Screens/routes: `/`, `/habits`, `/habits/new`, `/habits/[id]`, `/analytics`, `/journal`, `/review`, `/hall-of-fame`, `/pet`, `/identity`, and `/settings`.
- Pet Companion (`/pet`): a simple Tamagotchi. `lib/pet.ts` holds the character registry and pure feeding mechanics (each habit completed today earns one food token; feeding raises daily satiety; hunger resets per day). `lib/hooks/usePet.ts` exposes state via `useSyncExternalStore` over the `atomicly:pet` localStorage mirror; food is derived from real habit completions. `components/pet/PixelSprite.tsx` renders the pixel art.
