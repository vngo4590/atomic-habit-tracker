...
- **PR preview environments:** four additional workflows — `.github/workflows/pr-preview.yml` (deploy + Playwright), `pr-preview-teardown.yml` (close-driven cleanup), `pr-preview-reaper.yml` (hourly safety net), `pr-preview-open.yml` (reviewer-IP toggle), plus the one-time `pr-preview-bootstrap.yml`. They provision per-PR ephemeral stacks under `rg-atomicly-pr-<pr>-<sha7>` from `infra/preview.bicep`. They authenticate as a SEPARATE `pr-preview-atomicly` Azure AD app (`AZURE_PREVIEW_CLIENT_ID`) so they cannot touch the dev RG. See the `atomic-habit-pr-preview-env` skill for full details.
