...
.agents/skills/    # Project-local skills shared by all agents (edit here)
```
...
### Skills
- Edit skills only under `.agents/skills/` — this is the single shared location for all agents.
...
| `lib/repositories/reflection.ts` | Journal, weekly review, and (legacy) lesson-progress DB queries |
