...
8. **Habit stacking:** Habits can be linked into linear chains via `habit.stackAfterId`. The Stack tab on `/habits/[id]` lets users insert a habit before or after another habit. Circular dependencies are blocked by `wouldCreateCycle()` in `lib/stack.ts`. On the Today page, only the first undone habit in each chain is visible; checking it off reveals the next.
9. **localStorage:** Only used for `atomicly:theme`, `atomicly:accent`, `atomicly:onboarding-seen` — UI mirrors only, NOT source of truth for domain data.
...
| `lib/stack.ts` | Habit stacking logic: `getStackChain`, `getStackRoot`, `wouldCreateCycle`, `stackInsertPatches`, `stackRemovePatches`, `getVisibleStackHabit`, `groupHabitsByStack` |
| `components/StackTab.tsx` | Stack tab UI for habit detail page: habit selector, before/after toggle, stack diagram, circular-dependency error, remove button |
