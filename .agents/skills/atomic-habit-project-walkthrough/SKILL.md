...
components/        # Reusable client UI components
  ...
  StackDiagram.tsx    # Stack chain diagram with standalone-only picker + search + modal errors (habit detail Stack tab)
...
| `components/StackDiagram.tsx` | Horizontal stack chain diagram on the habit detail Stack tab; standalone-only picker (anchor can be any chain position) with search; routes all mutations through `store.applyStackMutation`; surfaces `StackError` messages in a `Modal` |
