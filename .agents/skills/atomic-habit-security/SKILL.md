...
| WAF policy (OWASP DRS, Bot Manager, rate limits) | `infra/modules/wafPolicy.bicep` |
...
- **New WAF rule:** add to `customRules.rules` in `wafPolicy.bicep`.
  `rateLimitDurationInMinutes` accepts only `1` or `5`. Managed rule sets are
  Premium-only — keep them gated behind `enableManagedRules`.
