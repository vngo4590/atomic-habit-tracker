...
resource errorRateSpikeAlert 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = {
  ...
  properties: {
    ...
    criteria: {
      allOf: [
        {
          query: 'AppExceptions | where TimeGenerated >= ago(5m)'
...
resource authFailureSpikeAlert 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = {
  ...
  properties: {
    ...
    criteria: {
      allOf: [
        {
          query: 'AppEvents | where TimeGenerated >= ago(5m) | where Name == "auth.login_failed"'
          timeAggregation: 'Count'
          operator: 'GreaterThan'
          threshold: 20
          failingPeriods: {
...
resource uncaughtExceptionAlert 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = {
  ...
  properties: {
    ...
    criteria: {
      allOf: [
        {
          query: 'union isfuzzy=true (AppExceptions | where TimeGenerated >= ago(5m) | where tolower(tostring(column_ifexists("HandledAt", ""))) == "unhandled"), (AppTraces | where TimeGenerated >= ago(5m) | where Message has "Unhandled exception" or Message has "uncaught exception")'
          timeAggregation: 'Count'
          operator: 'GreaterThan'
          threshold: 0
          failingPeriods: {
