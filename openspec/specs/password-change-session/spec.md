# password-change-session Specification

## Purpose
Define how the session-revocation gate behaves when a user changes their own password: the
initiating (current) device stays authenticated by refreshing its session issue time
(`authTime`) to at or after the revocation cutoff (`sessionsValidFrom`), while every OTHER
device is revoked. Also fixes the strict-comparison semantics of `isSessionRevoked` and the
`update`-trigger re-stamp in the JWT callback so a user can change their password repeatedly
in one session and wrong-password attempts report accurately (not as "not authenticated").
## Requirements
