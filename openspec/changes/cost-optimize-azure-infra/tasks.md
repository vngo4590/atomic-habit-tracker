...
- [ ] 1.4 In `infra/modules/postgres.bicep`, change `storageSizeGB: 32` → `storageSizeGB: 20` and `autoGrow: 'Enabled'` → `autoGrow: 'Disabled'`. Add a parameter so the value is reviewable in `main.bicep`.
