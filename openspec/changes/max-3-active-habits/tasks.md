...
- [ ] 1.1 Create `lib/habit-cap.ts` exporting `MAX_ACTIVE_HABITS = 3`, a pure
...
- [ ] 1.2 Add `lib/__tests__/habit-cap.test.ts` covering: no verdicts → all active; one `formed`
...
- [ ] 2.1 In `lib/repositories/habits.ts`, add async `countActiveHabits(userId, db)` counting
...
- [ ] 2.2 Change `createHabit` to return `Promise<CreateHabitResult>`: count active habits first
...
- [ ] 2.3 Add `lib/repositories/__tests__/habits-cap.test.ts` with a mock Prisma client covering:
...
- [ ] 3.1 Update `createHabitAction` in `lib/actions/domain.ts` to return `CreateHabitResult`
  (pass through the repository result; still revalidate on success). Verify: `npm run typecheck`.
- [ ] 3.2 Update `app/api/v1/habits/route.ts` POST to inspect the result and return HTTP 409 when
  `reason === "cap"` (otherwise 201 with the habit). Verify: route type-checks.
- [ ] 3.3 Update affected existing tests to the new result shape:
...
- [ ] 4.1 In `lib/store.ts` `addHabit`, handle the `CreateHabitResult`: on `{ ok:false, reason }`
...
- [ ] 4.2 Update store tests that resolve `createHabitAction` with a bare habit to resolve with
...
- [ ] 5.1 Create `components/HelpTip.module.css`: relative wrapper, a small circular `?` trigger,
  and an absolutely-positioned popover bubble. Comment each token's visual purpose. No inline
  static styles.
- [ ] 5.2 Create `components/HelpTip.tsx` (client component): a `<button type="button">` trigger
...
- [ ] 5.3 Add `components/__tests__/HelpTip.test.tsx` covering: hidden by default; click reveals
...
- [ ] 6.1 In `app/(root)/habits/new/page.tsx`, read `formationVerdicts` from the store, compute
...
- [ ] 6.2 Add the cap-banner class(es) to `app/(root)/habits/new/page.module.css` with comments.
- [ ] 6.3 Add `app/(root)/habits/new/__tests__/page.test.tsx` cap coverage (extend existing file):
...
- [ ] 7.1 Update `README.md` and `AGENTS.md`: document the max-3-active-habits rule
...
- [ ] 7.2 Run the full gate: `npm exec vitest run`, `npm run typecheck`, `npm run lint:app`,
